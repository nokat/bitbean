 ...
 * numeric operation). An excpetion being, that an input of 0x0000000000 would be rejected.
 * CScriptNum enforces those semantics by storing results as
...
bool VerifyScript(const CScript& scriptSig, const CScript& scriptPubKey, const CTransaction& txTo, unsigned int nIn, bool fStrictEncodings, int nHashType);
bool VerifySignature(const CTransaction& txFrom, const CTransaction& txTo, unsigned int nIn, bool fStrictEncodings, int nHashType);
