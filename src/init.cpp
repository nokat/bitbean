...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "BitBean:"))
                fCommandLine = true;
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 22460 or testnet: 25714)") + "\n" +
        ...
        "  -staking               " + _("Sprout your beans to support the network and grow new beans! (default: 1)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  BitBean is probably already running."), strDataDir.c_str()));
    ...
    if (fDaemon)
        fprintf(stdout, "BitBean server starting\n");
