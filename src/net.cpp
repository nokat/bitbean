...
unsigned int pnSeed[] =
{
    ...
    0xdb433f6c, 0xed685e77, 0x72894883, 0x77398789, 0x42ef4c90,
    0x7b404c90, 0x67bd8f9b, 0xb5d3afad, 0x439df5ad, 0x57ec1ead,
    0x8e084dad, 0x9302a5bc, 0x904a06bd, 0xebbe6002, 0x570e6302,
    0xe9ed0bcc, 0x88ddbed5, 0x207ec118, 0x7780dc18, 0x730c922e,
    0xb423ad32, 0x07542441, 0x5d3cd742, 0x54036844, 0x0017f345,
    0xee907446, 0xaa4ae447, 0x2ca58c49, 0x2f2c9b49, 0x88457e4c,
    0x47695a4c, 0xcff37650, 0x05a8df53, 0xa50ac658, 0x4cdce55a,
    0xc9a6125c, 0xbd213360, 0xc9ded162,
};
...
void static ThreadSproutMiner(void* parg)
{
    printf("ThreadSproutMiner started\n");
    CWallet* pwallet = (CWallet*)parg;
    try
    {
        vnThreadsRunning[THREAD_SPROUT_MINER]++;
        SproutMiner(pwallet);
        vnThreadsRunning[THREAD_SPROUT_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_SPROUT_MINER]--;
        PrintException(&e, "ThreadSproutMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_SPROUT_MINER]--;
        PrintException(NULL, "ThreadSproutMiner()");
    }
    printf("ThreadSproutMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_SPROUT_MINER]);
}
...
void ThreadOpenConnections()
{
    ...
    while (true)
    {
        ...
        // Add Beanstalk seed nodes if DNS seeders are all down (infastructure attack)
        if (addrman.size()==0 && (GetTime() - nStart > 60)) {
            static bool done = false;
            if (!done) {
                printf("Adding fixed seed nodes as DNS does not seem to be available.\n");
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. BitBean is probably already running."), addrBind.ToString().c_str());
        else
...
void StartNode(void* parg)
{
    ...
    if (!GetBoolArg("-sprout", true))
        ...
        if (!NewThread(ThreadSproutMiner, pwalletMain))
            printf("Error: NewThread(ThreadSproutMiner) failed\n");
}
...
bool StopNode()
{
    ...
    if (vnThreadsRunning[THREAD_SPROUT_MINER] > 0) printf("ThreadSproutMiner still running\n");
        MilliSleep(20);
    DumpAddresses();
    return true;
}
