    ...
    model(0)
{
    ...
    ui->lineEditBeanControlChange->setPlaceholderText(tr("Enter a BitBean address (e.g. 2JhbfkAFvXqYkreSgJfrRLS9DepUcxbQci)"));
#endif
...
void SendBeansDialog::setBalance(qint64 balance, qint64 sprout, qint64 unconfirmedBalance, qint64 immatureBalance)
{
    Q_UNUSED(sprout);
...
void SendBeansDialog::beanControlChangeEdited(const QString & text)
{
    if (model)
    {
        ...
        else if (!CBitbeanAddress(text.toStdString()).IsValid())
        {
            ...
            ui->labelBeanControlChangeLabel->setText(tr("WARNING: Invalid BitBean address"));
        }
        else
        {
