...
// Copyright (c) 2016 Bean Core www.bitbean.org
...
const QString BITBEAN_IPC_PREFIX("bitbean:");
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click bitbean: links)
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start bitbean: click-to-pay handler");
    else
...
{
    // clicking on bitbean: URLs creates FileOpen events on the Mac:
